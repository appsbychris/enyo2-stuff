enyo.kind({
	...
	components: [
		{name: "top", kind: "Slideable3d", axis: "v", unit: "%", min: -90, max: 0, classes: "enyo-fit slideable-sample top", onChange: "updateInfo"},
		{name: "right", kind: "Slideable3d", axis: "h", unit: "%", min: 0, max: 90, classes: "enyo-fit slideable-sample right", onChange: "updateInfo"},
		{name: "bottom", kind: "Slideable3d", axis: "v", unit: "%", min: 0, max: 90, classes: "enyo-fit slideable-sample bottom", onChange: "updateInfo"},
		{name: "left", kind: "Slideable3d", axis: "h", unit: "%", min: -90, max: 0, classes: "enyo-fit slideable-sample left", onChange: "updateInfo"}
	],
	handlers: {
	...
	populate: function(inSlideables) {
		...
		for (var s in inSlideables) {
			...
			slideable.createComponents([
				...
				{kind: "enyo.Checkbox", checked: true, ontap: "changeKeyframes", owner: this}, {content: "use KeyFrames"},
				{
					...
					info: {
						...
						useKeyframes: slideable.useKeyframes
					}
				}
			]);
		}
	},
	changeKeyframes: function(iS, iE) {
		if (iS.parent.kind == "Slideable3d") {
			iS.parent.useKeyframes = !iS.getChecked();
			this.updateInfo(iS.parent);
		}
	},
	updateInfo: function(inSender) {
		inSender.waterfallDown("onUpdateInfo", {
			...
			min: inSender.min,
			max: inSender.max,
			useKeyframes: inSender.useKeyframes,
